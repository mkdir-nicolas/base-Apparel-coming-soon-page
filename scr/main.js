const textError = {
  state: false,
  ui: `<span id="errorMessage">Please provide a valid email</span>`,
  getState: function () {
    return this.state;
  },
...
const addTextHelper = () => {
  if (!textError.getState()) {
    email.style.outlineColor = "var(--soft-red)";
...
const validateInput = () => {
  if (!isEmailValid()) {
    ...
    email.style.outlineColor = "var(--green)";
...
email.addEventListener("click", () => {
  email.style.outlineColor = "var(--dark-grayish-red)";
  if (!isEmailValid()) {
    removeTextHelper();
    email.classList.remove("error");
  }
